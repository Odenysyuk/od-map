class LineColoringGeneration {

    protected lineColoringFormat: LineColoringFormat;
    protected colorValues: number[] = [];    
    private minColorValues: number = 0; 
    private maxColorValues: number = 0;    

    constructor(colorGenerator: LineColoringFormat) {
        this.lineColoringFormat = colorGenerator;
    }

    public setlineColoringFormat(format: LineColoringFormat) {
        this.lineColoringFormat = format;
    }
 
    public setColorColumn(modelView: MapView[]) {
       this.colorValues = modelView.map(x => Number(x.LineColor)).filter(x => !isNaN(x)); 
